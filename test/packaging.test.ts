...
test("NixOS and Gentoo recipes consume the current revision root tarball", async () => {
  ...
  const gentoo = await readFile(new URL("gentoo/chatgpt-26.730.61639_p5.ebuild", repositoryPackaging), "utf8");
  for (const recipe of [nix, gentoo]) {
    assert.match(recipe, /upstream-26\.730\.61639-port\.5\/chatgpt\.tar\.gz/);
  }
