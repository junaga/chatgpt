---
name: computer-use
description: Control local Linux apps through Computer Use for tasks that require reading or operating app UI. Prefer purpose-built connectors, APIs, or CLIs when available.
---

## node_repl + @oai/sky (Computer Use)

* Use `node_repl` (JavaScript) for all Computer Use actions.
* Do not use other technologies besides `node_repl` for computer interactions, unless specifically requested by the user (e.g. `xdotool`, `ydotool`, direct portal calls, or shell-driven input).
...
The Linux client keeps the upstream app-shaped API and combines several native
backends. AT-SPI supplies app discovery, accessibility trees/diffs, semantic
clicks, editable and numeric values, selection, and exposed secondary actions.
On X11, screenshots and raw fallback use the bundled upstream `@oai/sky`
Linux client. On Wayland, screenshots use the Screenshot portal and raw
pointer, keyboard, drag, and scroll actions use a persistent compositor-approved
RemoteDesktop session with monitor stream geometry.

Screenshots contain the full desktop because the Linux Sky and portal APIs do
not expose a reliable app-window crop across mixed-scale displays. The
accessibility text remains scoped to the approved app.

The client never silently controls XWayland. A compositor that does not
implement RemoteDesktop (including some wlroots portal setups) returns an
explicit error. Portal screenshot coordinates can be compositor-specific on
mixed-scale monitor layouts, so prefer AT-SPI element actions. Apps with
incomplete AT-SPI support may expose fewer semantic elements.

Unlike the macOS app-window screenshot, Linux screenshots currently contain
full-desktop content. The X11 helper does not expose per-display origins/scales,
and portal screenshots do not supply enough mixed-scale transform metadata to
crop against AT-SPI bounds without risking the wrong region.
...
* No need to open or launch apps; `get_app_state` resolves desktop IDs, display names, and scanned `.desktop` paths, then launches the app without a shell if it is not already running.
* The `app` parameter may be an app's display name, freedesktop desktop ID, or scanned `.desktop` path.
* Do not call `list_apps` solely to resolve an identifier for a specific app. First, attempt `get_app_state` with the app's name.
* If an action or `get_app_state(...)` call fails when targeting an app by display name, immediately retry the same operation with that app's desktop application ID from `list_apps()` before pursuing other debugging paths.
...
if (state.screenshot) {
  await nodeRepl.emitImage({
    ...
    mimeType: state.screenshot.url.toLowerCase().match(/\.jpe?g(?:$|\?)/) ? "image/jpeg" : "image/png",
  });
}
```
